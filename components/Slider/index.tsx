...
const SliderComponent = ({ images, width, height }: SliderTypes) => {
  ...
  const toggleBullet = (prev: boolean) => {
    ...
    const nextSlide = prev ? Math.round(sliderRef.current.scrollLeft) - imgWidth : Math.round(sliderRef.current.scrollLeft) + imgWidth;
    ...
    <div>
      ...
      <div id="slider" className="slider relative mx-auto lg:mr-0" ref={sliderRef}>
        {images?.map((item: string, index: React.Key) => {
          return <Image key={index} priority={true} src={`/images/${item}`} alt="about-image" width={width} height={height} />;
        })}
      </div>
